...
This guide outlines the process for creating a new product page within the Rempah Warisan Bernam website. All product pages are located in this `/products` directory.
...
1.  **Duplicate the Template:**
    Make a copy of `meat-chicken-curry-powder.html` and rename it for the new product (e.g., `turmeric-powder.html`). Use lowercase and hyphens for the filename.
...
2.  **Update Product-Specific Metadata:**
    ...
    *   `<meta property="og:image">`: The absolute URL to the product's packaging image.
    *   `<meta property="og:url">`: The absolute URL to the new product page.
...
5.  **Update Google Sheet:**
    Ensure the new product is added to the **"RWB Products"** Google Sheet with its correct `product_name` and `link_to_product` (the WhatsApp purchase link). The `product_name` must be an exact match to the `CURRENT_PRODUCT_NAME` constant in the HTML file.
